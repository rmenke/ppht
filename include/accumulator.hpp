...
class accumulator {
  ...
  public:
    ...
    /// @brief Minimum number of points required to trigger a scan.
    Count min_trigger_points = 3;

    /// @brief Calculate parameters for scaling rho.
    ///
    /// Returns a pair containing the maximum scaled value rho can
    /// take on (height of the counter matrix) and the binary exponent
    /// by which rho should be scaled beforehand. The maximum scaled
    /// value is always an odd number.
    ///
    /// For the pair returned,
    /// @f$\rho_\text{scaled}=\rho_\text{unscaled}\cdot2^\mathtt{second}+\frac12(\mathtt{first}-1).@f$
    ///
    ...
    /// @brief Construct an instance of @ref accumulator.
    ///
    /// Given the dimensions of a bitmap, calculate the size of the
    /// quantized Hough space and scale the axis to make it as square
    /// as possible.
    ///
    ...
    accumulator(std::size_t rows, std::size_t cols,
                seed_t seed = std::random_device{}()) noexcept
    ...
    ///
    /// @return an iterator at the best candidate line, or @c end if
    ///   the range is empty
    ///
    template <class ForwardIt>
    static ForwardIt best_candidate(ForwardIt begin,
    ...
    std::optional<line> vote(point const &p) {
        ...
        //
        // (Technically, they are filled according to a hypergeometric
        // model, but that is prohibitively expensive to calculate and
        ...
        // We reject the null hypothesis as unlikely and return the
        // best line as a possible source of segments.
